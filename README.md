...
```bash
python3 scripts/closeDay.py --schedule
```

Одноразовое задание запомнит этот путь к Python и `closeDay.py`, поэтому
установленная команда `queue-close-day` для последующего срабатывания не нужна.
На Windows оно автоматически создаётся в Планировщике заданий, на Linux —
через службу `at`.
